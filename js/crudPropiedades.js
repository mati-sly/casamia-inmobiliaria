...
function validarYEnviar(operacion) {
   ...
   // CORREGIDO: Validación estricta de estado (como el original)
   if (!validarEstadoPropiedad()) {
        return false;
   }

   // Validar propietario (tanto para nuevo como editar)
...
function validarPropietario() {
    const propietarioSeleccionado = $('#propietario').val();
    
    if (!propietarioSeleccionado || propietarioSeleccionado.trim() === '') {
        mostrarMensaje('error', 'Propietario requerido', 'Debes seleccionar un propietario para la propiedad.');
        $('#propietario').focus();
        return false;
    }
...
// CORREGIDO: Validación estricta de estado (igual que el original)
...
// Funciones para botones de acción con URLs actualizadas
function editarPropiedad(idPropiedad) {
   Swal.fire({
       ...
       if (result.isConfirmed) {
           window.location.href = `crudPropiedades.php?idprop=${idPropiedad}`;
       }
   });
}
...
function gestionarImagenes(idPropiedad) {
   Swal.fire({
       ...
       if (result.isConfirmed) {
           window.location.href = `crudGestionarGaleria.php?idprop=${idPropiedad}`;
       }
   });
}
...
function confirmarEliminar(idPropiedad) {
   Swal.fire({
       ...
       if (result.isConfirmed) {
           window.location.href = `crudPropiedades.php?eliminar=${idPropiedad}`;
       }
   });
}
...
function cancelar() {
   Swal.fire({
       ...
       if (result.isConfirmed) {
           window.location.href = 'crudPropiedades.php';
       }
   });
}
