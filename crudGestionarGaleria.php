...
// Para testing - Usuario hardcodeado (puede ser admin u otro tipo)
...
// Obtener datos de la propiedad (SIN VALIDAR PERTENENCIA)
$verificar_propiedad = $conexion->prepare("SELECT p.titulopropiedad, u.nombres, u.apellidoPaterno FROM propiedades p JOIN usuarios u ON p.id_usuario = u.id WHERE p.idpropiedades = ?");
$verificar_propiedad->bind_param("i", $propiedad_id);
...
// Procesar acciones sobre imágenes existentes (SIN VALIDAR PERTENENCIA)
if (isset($_GET['accion'])) {
    ...
    switch ($accion) {
        case 'principal':
            ...
            // Marcar la seleccionada como principal (SIN VALIDAR PERTENENCIA)
        ...
        case 'eliminar':
            // Obtener nombre del archivo antes de eliminar (SIN VALIDAR PERTENENCIA)
...
<html lang="es">
<head>
    <title>CRUD Gestionar Galería - <?php echo htmlspecialchars($datos_propiedad['titulopropiedad']); ?></title>
    <meta charset="utf-8">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div>
                            📸 CRUD Gestionar Galería: <?php echo htmlspecialchars($datos_propiedad['titulopropiedad']); ?>
                            <small class="text-muted">
                                (Propietario: <?php echo $datos_propiedad['nombres'] . ' ' . $datos_propiedad['apellidoPaterno']; ?>)
                            </small>
                        </div>
                        <a href="crudPropiedades.php" class="btn btn-secondary">
                            ← Volver a CRUD Propiedades
                        </a>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <?php if ($total_imagenes_actual == 0): ?>
                            <div class="text-center py-5">
                                ...
                                <p>Sube la primera imagen de esta propiedad usando el formulario de arriba.</p>
                            </div>
                        <?php else: ?>
                            <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <ul>
                            ...
                            <li><strong>🛠️ CRUD:</strong> Puedes gestionar imágenes de cualquier propiedad desde el panel administrativo.</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function marcarPrincipal(idImagen) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `crudGestionarGaleria.php?idprop=<?php echo $propiedad_id; ?>&accion=principal&idimg=${idImagen}`;
                }
            });
        }
        ...
        function activarImagen(idImagen) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `crudGestionarGaleria.php?idprop=<?php echo $propiedad_id; ?>&accion=activar&idimg=${idImagen}`;
                }
            });
        }
        ...
        function desactivarImagen(idImagen) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `crudGestionarGaleria.php?idprop=<?php echo $propiedad_id; ?>&accion=desactivar&idimg=${idImagen}`;
                }
            });
        }
        ...
        function eliminarImagen(idImagen) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = `crudGestionarGaleria.php?idprop=<?php echo $propiedad_id; ?>&accion=eliminar&idimg=${idImagen}`;
                }
            });
        }
